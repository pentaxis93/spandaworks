...
Shared CLI command execution utilities for Talos Rust packages.

## Overview

`talos-cli` provides a clean, ergonomic API for wrapping external CLI tools in async Rust code. It handles the common patterns needed when building MCP servers that delegate to command-line utilities.
...
```toml
[dependencies]
talos-cli = { path = "../../../shared/utils/rust-cli" }
```
...
```rust
use talos_cli::run_command_stdout;
...
```rust
use talos_cli::run_command_with_stdin;

let email_body = "Hello from Talos!";
...
```rust
use talos_cli::run_command_stdout_lossy;
...
- *(Future)* Any Talos Rust package that wraps CLI tools
