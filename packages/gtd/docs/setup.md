...
Complete installation instructions for spanda-gtd.

## Prerequisites

### TaskWarrior 3.x

spanda-gtd requires TaskWarrior version 3.x (not 2.x).
...
**For Zsh:**
```bash
echo 'source ~/src/spanda-gtd/config/aliases.sh' >> ~/.zshrc
source ~/.zshrc
```

**For Bash:**
```bash
echo 'source ~/src/spanda-gtd/config/aliases.sh' >> ~/.bashrc
source ~/.bashrc
```
...
```json
{
  "mcpServers": {
    "spanda-gtd": {
      "command": "node",
      "args": ["/home/YOUR_USER/src/spanda-gtd/mcp-server/dist/index.js"]
    }
  }
}
```
...
```json
{
  "mcp": {
    "servers": {
      "spanda_gtd": {
        "type": "local",
        "command": ["node", "/home/YOUR_USER/src/spanda-gtd/mcp-server/dist/index.js"]
      }
    }
  }
}
...
node ~/src/spanda-gtd/mcp-server/dist/index.js
...
spanda-gtd is designed for TaskWarrior 3.x. Some features may work with 2.x but it's not officially supported. Upgrade if possible:
```bash
task --version
